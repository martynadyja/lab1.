...
try:
    for i in range(len(X2)):
        for j in range(len(Y2)):
            if Y2[j] == 0:
                print()
            elif (X2[i] % Y2[j]) == 0 and X2[i] % 2 == 0 and Y2[j] % 2 == 0:
                print("Found X is:", X2[i], "and found Y is:", Y2[j])

except:
    print("wystapilo dzielenie przez 0")
...
Z = 'X is divisible by Y.' if X3 % Y3 == 0 else 'X is not divisible by Y.'
XY = round(X3 / Y3, 2)
print("X/Y is:", XY, Z)
...
Z3 = X6 + Y6
ax = Axes3D(figure(figsize=(8,5)))
ax.plot_surface(X5, Y5, Z3, rstride=2, cstride=1, cmap=cm.spring, linewidth=0.4)
show()
